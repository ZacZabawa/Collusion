...
function renderGraph(graphId, graphData) {
    // Initial setup
    let width = 0.95 * window.innerWidth; // 90% of viewport width
    ...
    if (!svg.node()) {
        ...
        var text = node.append("text")
            ...
            .attr("dy", ".35em")
            .style("font-size", `${svgWidth / 10}px`) // Adjusted for bigger and responsive font size
            .text(d => {
...
function loadSvgPatterns(svg) {
    const patterns = ['organization', 'function', 'project'];
    ...
    svg.append('defs')
        ...
        .append('image')
        .attr('xlink:href', d => `{{ site.assets.svg_path }}/${d}.svg`)
        .attr('width', 120)
        .attr('height', 120);
}
